...
void overlay()
{
  ...
  //TString ttfile = "hst_ttjets.root";
  //TString dyfile = "hst_DYM50.root";
  
  TString ttfile = "hst_Zqq.root";
  TString dyfile = "hst_Wqq.root";
  ...
  TString plotname1 = "mass_jj";
  TString plotname2 = "mass_jj"; 
  ...
  TFile *filett = new TFile(ttfile);
  TFile *filedy = new TFile(dyfile);

  //Now open the respective histograms from the files
  TH1F *htt = (TH1F*)filett->Get(plotname1);
  TH1F *hdy = (TH1F*)filedy->Get(plotname2);
  ...
  decorate(hdy,xtitle,ytitle,"",kBlue,2,kBlue-7,20,1);
  decorate(htt,xtitle,ytitle,"",kRed,2,kGreen+2,21,0);
  ...
  */
  ...
  lg1->AddEntry(hdy,"Z/#gamma* M>50 GeV","lf"); // Added the two entries for the two histograms
  lg1->AddEntry(htt,"t#bar{t}","lp");           // we shall be drawing.
